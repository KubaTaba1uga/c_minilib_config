...
struct cmc_ConfigParseInterface {
  ...
  cme_error_t (*create)(cmc_ConfigParserData *);
  // Decide if parser can parse a file based on it's path
  cme_error_t (*is_format)(const size_t n, const char path[n], bool *result);
  // Parse a file based on it's path
  cme_error_t (*parse)(const size_t n, const char path[n],
