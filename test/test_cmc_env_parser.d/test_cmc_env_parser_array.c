...
void test_required_array_without_default_should_fail_always(void) {
  ...
  err = cmc_field_create("", cmc_ConfigFieldTypeEnum_INT, NULL,
...
void test_optional_array_parsing(void) {
  ...
  err = cmc_field_create("optional_array", cmc_ConfigFieldTypeEnum_ARRAY,
                         NULL, /* no default */
                         true, /* optional */
                         &f_array);
  TEST_ASSERT_NULL(err);
...
void test_double_nested_array_parsing(void) {
  ...
  for (uint32_t i = 0; i < outer->_self.subnodes_len; ++i) {
    struct cmc_ConfigField *lvl1 = cmc_field_of_node(outer->_self.subnodes[i]);
    for (uint32_t j = 0; j < lvl1->_self.subnodes_len; ++j) {
      struct cmc_ConfigField *lvl2 = cmc_field_of_node(lvl1->_self.subnodes[j]);
